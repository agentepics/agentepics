...
description: "How to run commands and bundle executable scripts in your epics."
---

Epics can instruct agents to run shell commands and bundle reusable scripts in a
`scripts/` directory. This guide covers one-off commands, self-contained scripts
with their own dependencies, and how to design script interfaces for agentic
use across phases, plans, and runtime behavior.

## One-off commands

When an existing package already does what you need, you can reference it
directly in your `SKILL.md` or `EPIC.md` guidance without a `scripts/`
directory. Many ecosystems provide tools that auto-resolve dependencies at
runtime.
...
**Tips for one-off commands in epics:**

- **Pin versions** (e.g., `npx eslint@9.0.0`) so the command behaves the same over time.
- **State prerequisites** in your epic instructions (for example in `SKILL.md`, `EPIC.md`, or a phase description) rather than assuming the agent's environment has them. For compatibility-level requirements, use the [`compatibility` frontmatter field](/skill-specification#compatibility-field).
...
## Referencing scripts from an epic

Use **relative paths from the epic directory root** to reference bundled files.
The agent resolves these paths automatically — no absolute paths needed.

List available scripts in the epic materials that need them so the agent knows
they exist. This may be in `SKILL.md`, `EPIC.md`, a phase description, a plan,
or a runtime file.
...
plan files, and runtime files. Script execution paths are relative to the
**epic directory root**.
</Note>
...
own dependencies inline. The agent can run the script with a single command —
no separate manifest file or install step required.
...
In an epic, the same script might be used from several places:

- a phase definition in `EPIC.md`
- a current plan in `plans/`
- a helper instruction in `skills/`
- a recurring job in `cron.d/`
- an event handler in `hooks/`
...
<Note>
For a real-world example of an agentic validation script, see the
[`validate-epic.sh` validator](/epic-creation/create-epic-skill#the-validator-validate-epicsh)
bundled with the create-epic skill. It demonstrates structured output, meaningful
exit codes, and clear error messages — all designed for agent consumption.
</Note>
