...
4. Update the embedded footer source in
   [epics.sh/internal/epic/skill_footer.go](../../epics.sh/internal/epic/skill_footer.go).
5. Update any `epics.sh` docs that describe the footer marker, footer refresh
   flow, or validation behavior.
6. Run `epics upgrade-skill-footer` against the maintained fixture epics in
   `epics.sh/examples/fixtures/` that embed the footer.

## Curated epics repo

7. Run `epics upgrade-skill-footer` against every published epic in the
   `epics` repo so the local `SKILL.md` copies match the canonical footer
   exactly.

## Generated copies

8. Refresh any generated or embedded copies:
   - `epics.sh/examples/fixtures/*/SKILL.md`
   - `epics.sh/registry/epics/*.json` `skillMd` snapshots

## Validation

9. Run the relevant `epics.sh` tests, at minimum:
   - `go test ./internal/epic ./internal/cli`
10. Run `epics validate` against at least:
   - one curated epic from the `epics` repo
   - one fixture epic from `epics.sh/examples/fixtures/`
11. Confirm the canonical footer URL and any linked installer URL return live
    content from GitHub after push.

## Versioning

12. If the footer text changes but the EPIC spec version does not, keep the
    version constant and still refresh all propagated copies.
13. If the marker semantics or validation contract change, document the
